...
class GalleryBuilder:
    def __init__(
        ...
        self.languages = self.site_config.get("languages") or [
            self.site_config.get("lang", "fr")
        ]
    ...
    def build_gallery(self):
        ...
        if self.unilingual:
            ...
            prefix = (
                "/assets/gallery_images/"  # Préfixe pour accéder aux images copiées
            )
            ...
            context = {
                ...
                "page": {
                    "lang": lang,
                    "url": gallery_url,
                    "title": self.translations.get(lang, {}).get(
                        "gallery_title", "Gallery"
                    ),
                },
            }
            rendered = self.jinja_env.get_template(self.gallery_template).render(
                **context
            )
    ...
    def _render_gallery_page(
        self, output_gallery_dir, gallery_files, prefix, page_info
    ):
        """
        Rendu de la page principale de la galerie.
        """
        logging.info("Final gallery_files passed to template: %s", gallery_files)
    ...
    def _render_image_pages(
        self, output_gallery_dir, images_dir, image_files, prefix, page_info
    ):
        """
...
if __name__ == "__main__":
    ...
    gallery_builder = GalleryBuilder(
        Path("src"), Path("dist"), jinja_env, {"lang": "fr", "multilang": True}, {}
    )
